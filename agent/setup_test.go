...
func TestInitBuildsJalonNotTheTarget(t *testing.T) {
	script := emit(t, InitOptions{
		Repo: "/home/agent/site.example", Jalon: "/home/agent/jalon", User: "u",
	})
	unit := heredoc(t, script, ".service <<'UNIT'", "UNIT")
	...
	for _, want := range []string{
		...
		"ReadWritePaths=/home/agent/site.example /home/agent/jalon",
	} {
		if !strings.Contains(unit, want) {
			t.Errorf("the unit is missing %q:\n%s", want, unit)
		}
	...
	if strings.Contains(unit, "make -C /home/agent/site.example") {
		t.Errorf("the unit tries to build the target:\n%s", unit)
	}
	if !strings.Contains(unit, "WorkingDirectory=/home/agent/site.example") {
