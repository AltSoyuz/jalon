...
| `agent_cmd.go` | The only bridge to `agent/`: the `doctor`, `review`, `work`, `recap` and `agent-init` verbs, flag parsing and nothing else. |
...
- **The agent layer is removable.** `rm -rf agent agent_cmd.go`, drop five cases
