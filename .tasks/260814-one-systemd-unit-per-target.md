...
- 2026-08-14 altsoyuz: found while adding altsoyuz.com as a second target on a server already running jalon, before the root block was executed. The directory name is sanitised, since a name systemd refuses to load would only be discovered at install time.
